...
--
simbadObjectTypes :: Html -> Html
simbadObjectTypes desc =
...
nameInfo =
  ...
  simbadObjectTypes "the SIMBAD database"
  <> ". "
...
renderTypes objs = 
  let toRow (sti,n) = tr (tdObject sti <> tdNum n)
      ...
      pText = 
        ...
        <> toLink StaticHtml "/search/dtype/"
        ("the SIMBAD dendogram view" :: Html)
        <> "."
...
renderDependency objs = 
  let xs = toTree (addCode objs)
      ...
      pText =
        ...
        <> toLink StaticHtml "/search/type/"
        ("the SIMBAD types view" :: Html)
        <> "."
