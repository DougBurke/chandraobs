...
import Rel8 (Insert(..), Result, OnConflict(Abort), Returning(NoReturning)
            , insert, lit, run_, values
            )
...
getObsCatQuery oids = 
  let ois = map (show . fromObsIdVal) oids
...
toSOE m con = do
  ...
  pure ScienceObs {
    ...
    -- , soMultiTelObs = multiTelObs     TODO WAT TODO
...
ocatToNonScience m = do
  ...
  let target = T.pack ("CAL-ER (" <> show (fromObsIdVal obsid) <> ")")
      rTime = appExp
...
queryOCAT oids = do
  ...
  ansE <- makeObsCatQuery True oids
...
addProposal conn pnum = do
  ...
  let req = NHC.parseRequest_ baseLoc
      ...
      errPrint = T.hPutStrLn stderr

  rsp <- getTextFromOCAT req1
  case rsp of
    Right ans -> extractAbstract conn pnum ans
...
runInsert ::
  ...
  -> Insert a
  -> IO ()
runInsert conn now ins = do
  let stmt = insert ins {- >> updateLastModified now -}
  res <- S.run (S.statement () (run_ stmt)) conn
  case res of
    Right ans -> pure ()
    Left err -> quickError ("Database error on insert: " <> T.pack (show err))
...
extractAbstract conn pnum txt = do
  let printErr = T.hPutStrLn stderr
  now <- getCurrentTime
  case findAbstract pnum txt of
    Left emsg -> do
      ...
      runInsert conn now $ Insert
        { into = missingProposalAbstractSchema
        , rows = values [lit missingAbs]
        , onConflict = Abort
    ...
    Right (absTitle, absText) -> do
      ...
      runInsert conn now $ Insert
        { into = proposalAbstractSchema
        , rows = values [lit newAbs]
        , onConflict = Abort
        , returning = NoReturning
        }
      pure True
